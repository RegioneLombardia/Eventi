...
<div class="user-import-task-search my-2 collapse" id="form-search-events">
    ...
    <div class="neutral-2-bg p-4">
        <div class="row">
            ...
            <!-- task_date -->
            <div class="col-lg-6">
                <?= $form->field($model, 'taskFrom')->widget(DateControl::className(), [
                    'type' => DateControl::FORMAT_DATE,
                    'options' => [
                        'layout' => '{input} {picker} ' . (($model->task_date == '') ? '' : '{remove}')
                    ]
                ])->label(Module::t('amosuserimporter', 'Dal'));; ?>
            </div>
            <div class="col-lg-6">
                <?= $form->field($model, 'taskTo')->widget(DateControl::className(), [
                    'type' => DateControl::FORMAT_DATE,
                    'options' => [
                        'layout' => '{input} {picker} ' . (($model->task_date == '') ? '' : '{remove}')
                    ]
                ])->label(Module::t('amosuserimporter', 'Al')); ?>
            </div>
