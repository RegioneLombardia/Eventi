...
use amos\userimporter\models\search\UserImportTaskSearch;
...
class UserImportTaskController extends CrudController
{
    ...
    public function actionCreate()
    {
        ...
        $this->model->task_date               = Yii::$app->formatter->asDate('now',
            'php:Y-m-d H:i:s');
        $this->model->user_id                 = CurrentUser::getUser()->getId();
        $this->model->event_group_referent_id = EventsUtility::getCurrentDG()->id;
        $this->model->status                  = UserImportTaskStatus::PENDING;

        $isCreateList = \Yii::$app->request->get('create_list');
        if($isCreateList){
            $this->model->setScenario(UserImportTask::SCENARIO_CREATE_LIST);
            $this->model->is_from_edit_list = 1;
        }

        if ($this->model->load(Yii::$app->request->post()) && $this->model->validate()) {
            if ($this->model->save()) {
                if($isCreateList) {
                    $this->model->importFromDbList();
                }
