...
//             else if ($code != 200 && $code != 302) throw new MailUpException($code, "Unknown error");
     ...
     * @return mixed
     */
    public function getUnsubscribedByList($id, $params = [])
    {
        $this->oauth2Autentication();
        $params = $this->adjustPageNumber($params);
        $url = Url::toRoute(ArrayHelper::merge([$this->getConsoleEndpoint() . "/Console/List/$id/Recipients/Unsubscribed"], $params), 'https');
        return json_decode($this->callMethod($url, "GET", null, "JSON"));
    }
     ...
     * @return mixed
     */
    public function getEmailOptinsByList($id, $params = [])
    {
        $this->oauth2Autentication();
        $params = $this->adjustPageNumber($params);
        $url = Url::toRoute(ArrayHelper::merge([$this->getConsoleEndpoint() . "/Console/List/$id/Recipients/EmailOptins"], $params), 'https');
        return json_decode($this->callMethod($url, "GET", null, "JSON"));
    }
