...
class ElasticQuery extends \yii\base\BaseObject
{
     ...
     * @param string $type
     */
    public function query($query = null, $index = '_all', $type = null, $sort = [])
    {
...
//            'functions' => [],
    ...
    protected function buildQuery()
    {
        ...
        if (count($this->sort)) {
//            $sort                = $this->addOrderBy($this->sort);
...
//        pr($this->query);
//        pr(\yii\helpers\Json::encode($this->query));
//        die;
...
//pr($query_result);
     ...
     * @return $this
     */
    public function addMust($field, $condition, $operation = 'match', $analyzer = "", $boostFields = [])
    {
        if (is_array($condition)) {
            foreach ($condition as $value) {
                $item = new ItemCondition(['field' => $field, 'condition' => $value, 'operation' => $operation]);
                $item->setAnalyzer($analyzer);
                $item->setFields($boostFields);
                $this->must[] = $item->toArray();
            }
        } else {
            $item = new ItemCondition(['field' => $field, 'condition' => $condition, 'operation' => $operation]);
            $item->setAnalyzer($analyzer);
            $item->setFields($boostFields);
            $this->must[] = $item->toArray();
        }
    ...
    {
        $this->addMust($field, self::WILDCARD_MORE . $condition . self::WILDCARD_MORE, 'wildcard');
        return $this;
    }
