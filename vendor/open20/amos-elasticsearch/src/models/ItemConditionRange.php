...
class ItemConditionRange extends BaseItemCondition
{
    ...
    public function toArray()
    {
        ...
        if (!is_null($this->start_date)) {
            if ($this->getUseDate()) {
                $condition [$this->field] [$this->from_condition] = date(self::DATE_FORMAT, $this->start_date);
            } else {
        ...
        if (!is_null($this->end_date)) {
            if ($this->getUseDate()) {
                $condition [$this->field] [$this->to_condition] = date(self::DATE_FORMAT, $this->end_date);
            } else {
