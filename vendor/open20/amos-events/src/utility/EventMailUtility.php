...
//        return self::sendEmailGeneral($from, [$email], $subjectWithParams, $textWithParms);
    ...
    public static function parseEmailWithParams($event, $user_id, $text, $setStrong = true)
    {
        ...
        if ($event) {
            ...
            if($logoEmail){
                $url = \Yii::$app->params['platform']['backendUrl'] . $logoEmail->getWebUrl();
            }else if ($logo) {
...
//                    $urlLanding.= "?language=en-GB";
//                    $urlLandingNoTracking.= "?language=en-GB";
//                }
//            }
            ...
            if (is_null($user_id)) {
                $textWithParms = AmosEvents::t('amosevents', $text, [
                        ...
                        'DATA_INIZIO' => self::setStrong(self::formatDateWithTimeZone($event->begin_date_hour,'d/m/Y'), $setStrong),
                        'DATA_FINE' => self::setStrong(self::formatDateWithTimeZone($event->end_date_hour,'d/m/Y'), $setStrong),
                        'ORA_INIZIO' => self::setStrong(self::formatDateWithTimeZone($event->begin_date_hour, 'H:i'), $setStrong),
                        'ORA_FINE' => self::setStrong(self::formatDateWithTimeZone($event->end_date_hour, 'H:i'), $setStrong),
                        'DATA_CHIUSURA_ISCRIZIONI' => self::setStrong(self::formatDateWithTimeZone($event->registration_date_end, 'd/m/Y'), $setStrong),
                        'ORA_CHIUSURA_ISCRIZIONI' => self::setStrong(self::formatDateWithTimeZone($event->registration_date_end, 'H:i'), $setStrong),
                        ...
                        'DATA_INIZIO_STREAMING' => $event->eventLanding ? self::setStrong(self::formatDateWithTimeZone($event->eventLanding->date_begin_streaming,'d/m/Y'), $setStrong) : '',
                        'ORA_INIZIO_STREAMING' => $event->eventLanding ? self::setStrong(self::formatDateWithTimeZone($event->eventLanding->date_begin_streaming,'H:i'), $setStrong) : '',
            ...
            $textWithParms = AmosEvents::t('amosevents', $text, [
                    ...
                    'EMAIL' => self::setStrong('[email]', $setStrong),
                    'TOKEN' => '[token]',
                    'MESE_CORRENTE'=> self::setStrong(\Yii::t('amosevents',$today->format('F'))),
                    'EVENTI_MESE_CORRENTE' => self::getEventiMeseCorrente(),
                ]
            );
        }
     ...
     */
    public static function getEventiMeseCorrente(){
        $modelSearch = new EventSearch();
        $dataProvider = $modelSearch->cmsPublishedSearch(['day' => 'this_month']);
        ...
        if(!empty($events)){
            ...
            foreach ($events as $event){
                $url = EventsUtility::getUrlLanding($event);
                $str .= "<li><a href='$url'>$event->title</a></li>";
            }
            $str .= '</ul>';
        }
        return $str;
    ...
    public static function formatDateWithTimeZone($date, $format = 'd/m/Y H:i')
    {
        if (!empty($date)) {
    ...
    public static function sendEventPublishedEmail($event)
    {
        ...
        $tos = array_filter($tos, function($value) { return !is_null($value) && $value !== ''; });
    ...
    public static function sendEmailEnableOperator($groupReferentMm, $user_id)
    {
        ...
        if ($operator) {
            ...
            $text = AmosEvents::t('amosevents', "L'utente <strong>{NomeCognome}</strong> della DG {dg} ha completato il percorso formativo ed è pronto ad utilizzare il sistema per la gestione degli eventi di Regione Lombardia.", [
                'NomeCognome' => $operator->userProfile->nomeCognome,
                'dg' => $dg ? $dg->denominazione : ''
            ]);
...
Potrai inoltre importare utenti e inserirli nel tuo database. Per accedere al backoffice ed iniziare a creare eventi, clicca <a href='{link}'>qui</a>", [
    ...
    public static function getFooterType($footerType)
    {
        ...
        $footerDisiscriviti = "<p style=\"text-align:center;line-height:1.2;word-break:break-word;margin:0\">" . AmosEvents::t('amosevents', "<a href='$linkDisiscriviti'>Desidero cancellarmi dalla piattaforma $appName</a>") . "";
    ...
    /**
     * Used in mobile bridge for api preference
     * @param $event_id
     * @param $data
     * @return array
     */
    public static function sendEmailNotifyEventPreference($event_id, $data)
    {
        $module = \Yii::$app->getModule('events');
        $groupAdminId = null;
        if ($module) {
            $groupAdminId = $module->groupReferentAdministration['id'];
        }
        $event = Event::findOne($event_id);
        if ($event) {
            $group_id = $event->event_group_referent_id;
            $queryOperators = User::find()
                ->innerJoin('event_group_referent_mm', 'event_group_referent_mm.user_id = user.id')
                ->andWhere(['event_group_referent_id' => $group_id])
                ->andWhere(['exclude_from_query' => 1]);

            // se la dg è diversa da UO comunicazione invio sia alla dg di appartenenza che lla dg stessa
            if ($group_id != $groupAdminId) {
                $operators = User::find()
                    ->innerJoin('event_group_referent_mm', 'event_group_referent_mm.user_id = user.id')
                    ->andWhere(['event_group_referent_id' => $groupAdminId])
                    ->andWhere(['exclude_from_query' => 1])
                    ->union($queryOperators)->all();
            } else {
                $operators = $queryOperators->all();
            }
        ...
        $pubblicationDate = '';
        if (!empty($data['landing_pubblication_date'])) {
            $date = new \DateTime($data['landing_pubblication_date']);
            $pubblicationDate = $date->format('d/m/Y H:i');
        }


        $i = 0;
        foreach ($operators as $userOperator) {
            $subject = AmosEvents::t('amosevents', "Evento pubblicizzato in una Landing page di Lombardia informa", [
            ]);


            $text = AmosEvents::t('amosevents', "Gentile {NomeCognome},
            <br>l'evento <strong>{event}</strong> è stato pubblicizzato nel servizio di Lombardia Informa nella landing page <a href='{landing_url}'><strong>{title_landing}</strong></a> pubblicata in data <strong>{landing_pubblication_date}</strong>.", [
                'NomeCognome' => $userOperator->userProfile->nomeCognome,
                'event' => $event->title,
                'title_landing' => $data['landing_title'],
                'landing_url' => $data['landing_url'],
                'landing_pubblication_date' => $pubblicationDate,
            ]);
            $ok = self::sendEmailGeneral($from, $userOperator->email, $subject, $text);
            if ($ok) {
                $i++;
            }
     ...
     * @return array
     */
    public static function sendEmailNotifyCampainPreference($event_id, $data)
    {
        $module = \Yii::$app->getModule('events');
        $groupAdminId = null;
        if ($module) {
        ...
        $sendDate = '';
        if (!empty($data['campaign_send_date'])) {
            $date = new \DateTime($data['campaign_send_date']);
        ...
        $i = 0;
        foreach ($operators as $userOperator) {
            $subject = AmosEvents::t('amosevents', "Evento pubblicizzato in una Campagna di Lombardia informa", [
            ]);

            $text = AmosEvents::t('amosevents', "Gentile {NomeCognome},
            <br>l'evento <strong>{event}</strong> è stato pubblicizzato nel servizio di Lombardia Informa nella Campagna <strong>{campaign}</strong> (canale {channel}) inviata in data <strong>{campaign_send_date}</strong>.", [
                'NomeCognome' => $userOperator->userProfile->nomeCognome,
                'event' => $event->title,
                'campaign' => $data['campaign_name'],
                'channel' => $data['channel'],
                'campaign_send_date' => $sendDate,
            ]);
            $ok = self::sendEmailGeneral($from, $userOperator->email, $subject, $text);
            if ($ok) {
                $i++;
