    ...
    public function actionDelete($id)
    {
        ...
        if ($this->model) {
            ...
            if (count($participantsAssociated) > 0 || count($companionsAssociated) > 0) {
                Yii::$app->getSession()->addFlash('danger', BaseAmosModule::t('amoscore', 'Alcuni partecipanti o partecipanti aggiuntivi hanno la lista di accreditamento che si voleva eliminare associata. Per eliminarla, rimuovere le associazioni presenti.'));
            } else {
                $this->model->delete();
