...
use open20\amos\events\commands\InvitationSenderController;
...
class EventController extends base\EventController
{
    ...
    public function behaviors()
    {
                    ...
                    'rules' => [
                        ...
                        [
                            ...
                            'actions' => [
                                ...
                                'test',
                                'get-comuni-ajax',
                                'get-comune-from-string',
                                'get-provincia-from-string',
                                'search-events',
                                'rate-event',
                                'force-download-ics',
                            ],
        ...
        $cachePage = EventsUtility::eventConfigCachePage();
        if($cachePage) {
            $behaviors['pageCache'] = $cachePage;
        }
        return $behaviors;
    }
    ...
    public function init()
    {
        ...
        if(\Yii::$app->request->isAjax) {
            \yii\base\Event::on(\app\modules\cms\base\CmsView::className(), AmosView::EVENT_AFTER_RENDER, function ($e) {
                foreach($e->sender->assetBundles as $key => $bundle) {
                    if ($key !== 'open20\amos\favorites\assets\FavoritesAsset') {
                        unset($e->sender->assetBundles[$key]);
                    }
                }
            ...
            if ($moduleEvent->enableNewWizard) {
                ...
                if (!empty($event->eventEmailTemplates)) {
                    ...
                    if ($event->multilanguage) {
                        ...
                        $eventTemplateEng = EventEmailTemplates::findOne($eventTemplate->id);
                        $eventEng = Event::findOne($event->id);
                        if (!empty($eventTemplateEng->$text_attribute)) {
                            $ticketTextEng = EventMailUtility::parseEmailWithParams($eventEng, $user->id, $eventTemplateEng->$text_attribute);
                            $ticketText .= "<hr><br>" . $ticketTextEng;
                        }
    ...
    public function actionSendTicketsMassive($eid)
    {
        ...
        if ($event && !empty($event)) {
            ...
            $invitations = $eventInvitationModel::find()
                ...
                ->andWhere(['event_invitation.is_ticket_sent' => 0])
                ...
                ->andWhere(['community_user_mm.status' => 'ACTIVE'])
     ...
     *
     */
    public function actionDownloadGuideFile()
    {
        $path = Yii::getAlias('@vendor') . '/open20/amos-events/src/downloads';
        $file = $path . '/Guida_compilazione.pdf';
        if (file_exists($file)) {
            Yii::$app->response->sendFile($file);
        }
    }
...
//                                    ($participantsWantToJoin > ($event->seats_available - $event->checkParticipantsQuantity())) ||
                                ...
                                if ($user && $user->userProfile->attivo) {
                                    ...
                                    // creo gli partecipanti aggiuntivi/memebri del gruppo
                                    $companions = [];
...
//                }
     ...
     * @throws \yii\base\InvalidConfigException
     */
    public function actionGetComuniAjax($q = null, $id = null)
    {
        \Yii::$app->response->format = Response::FORMAT_JSON;
        ...
        $all = $comuni->andWhere(['LIKE', 'istat_comuni.nome', $q])->all();
        foreach ($all as $comune) {
            $results [] = ['id' => $comune->id, 'text' => $comune->nome];
        }
        $out['results'] = $results;
        return $out;
    }
     ...
     * @throws \yii\base\InvalidConfigException
     */
    public function actionGetComuneFromString($comune)
    {
        $moduleEvents = \Yii::$app->getModule('events');
        \Yii::$app->response->format = Response::FORMAT_JSON;
        $strComune = null;
        $explode = explode(',', $comune);
        if (!empty($explode[0])) {
            $str = $explode[0];
            $exploded = explode(' ', $str);
            if (!empty($exploded[0])) {
                $strComune = $exploded[0];
            }
        }
        $model = IstatComuni::find()
            ->andWhere(['nome' => $strComune])->one();
     ...
     * @throws \yii\base\InvalidConfigException
     */
    public function actionGetProvinciaFromString($provincia)
    {
        $moduleEvents = \Yii::$app->getModule('events');
        \Yii::$app->response->format = Response::FORMAT_JSON;
        $strProvincia = null;
        $explode = explode(',', $provincia);
        if (!empty($explode[0])) {
            $str = $explode[0];
            $exploded = explode(' ', $str);
            if (!empty($exploded[0])) {
                $strProvincia = $exploded[0];
            }
        }
        $model = IstatProvince::find()
            ->andWhere(['sigla' => $strProvincia])->one();
    ...
    /**
     * @param null $currentComune
     * @param null $day
     * @param null $tagPreference
     * @return string
     */
    public function actionSearchEvents($currentComune = null, $day = null, $tag_id = null)
    {
        return $this->renderAjax('search-events', [
            'currentComune' => $currentComune,
            'day' => $day,
            'tagPreference' => $tag_id
        ]);
    }
     ...
     * @param int $user_id
     */
    public function actionTestNotifica($notification_id = 189, $user_id = 435)
    {
        $list = EventPushNotification::findOne($notification_id);
        if ($list) {
            $list->status = 'draft';
            $list->save(false);
        }

        EventPushNotificationSent::deleteAll(['user_id' => $user_id, 'event_push_notification_id' => $notification_id]);
        $controller = new InvitationSenderController('invitation-sender', \Yii::$app->getModule('events'));
        $controller->actionSendPushNotification();
        die;
    }
     ...
     * @return string
     */
    public function actionCompanions($id)
    {
        $this->setUpLayout('form');
        ...
        $invitation = EventInvitation::find()->andWhere(['user_id' => \Yii::$app->user->id, 'event_id' => $id])->one();
        if ($invitation) {
            $nCompanions = $invitation->getCompanions()->andWhere(['event_participant_companion.deleted_at' => null])->count();
        }else{
            throw new NotFoundHttpException("Non sei iscritto a questo evento.");
        }

        if (\Yii::$app->request->isPost) {
            $enable_companions = true;
            $n_companions = \Yii::$app->request->post('n_companions');
            if($n_companions == 'delete_companions'){
                $enable_companions = false;
            }
            if ($enable_companions) {
                $eventControllers = new EventController('event', \Yii::$app->getModule('events'));
                ...
                foreach ($companions as $companion) {
                    $eventControllers->addCompanion($this->model->id, $invitation, $companion);
                }
            } else {
                EventParticipantCompanion::deleteAll(['event_invitation_id' => $invitation->id]);
            }

            \Yii::$app->session->addFlash('success', AmosEvents::t('amosevents', "Partecipanti aggiuntivi modificati con successo."));
            return $this->redirect(EventsUtility::getUrlLanding($this->model));
        }
        return $this->render('companions', ['model' => $this->model, 'nCompanions' => $nCompanions]);
    }
     ...
     * @return false|string
     */
    public function actionRateEvent($id, $rating, $user_id, $user_ip)
    {
        \Yii::$app->response->format = Response::FORMAT_JSON;
        ...
        if (!Yii::$app->user->isGuest) {
            ...
            $review = EventLandingRating::findOne(['event_id' => $id, 'user_id' => $userId]);
            if (empty($review)) {
                $review = new EventLandingRating();
            }
            ...
            if (is_array($arrayCookieValues)) {
                $arrayCookieValues[$event->id] = $review->id;
            } else {
                $attr = $event->id;
                $arrayCookieValues->$attr = $review->id;
            }
            $jsonValues = json_encode($arrayCookieValues);
