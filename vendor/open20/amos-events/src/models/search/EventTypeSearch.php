...
class EventTypeSearch extends EventType
{
    ...
    {				
        ...
        $query = $eventTypeModel::find()->andWhere(['event_context_id' => EventTypeContext::EVENT_TYPE_CONTEXT_GENERIC]);
        if(!\Yii::$app->user->can('EVENT_WEBEX_CREATOR')){
                $query = $query->andWhere(['<>','id',EventType::WEBEX_EVENTS]);
        }
		$query = $query->orderBy('order, id');
        return $query;
    }
