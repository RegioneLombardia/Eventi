...
$event_id = $model->id;
$max_participant = 100;
$type = SpecificUserEventSearch::TYPE_BASE;
if($communication) {
    $type = SpecificUserEventSearch::TYPE_COMUNICATION;
}
...
$(document).on('keyup', '#search-name-id',function(){
    ...
    let internal_list_id = $('#internal-list-id').val();
    var name = $(this).val();
    
    if(/*name.length > 3*/ true){
          $.ajax({
                url: '/events/wizard/search-users-nominal',
                type: 'GET',
                data: { name: $(this).val() ,event_id: $event_id, selected_users: selected, deleted_users: deleted, internal_list_id: internal_list_id, typeSearch: '$type', showAllUsers : '$showAllUsers'},
...
$(document).on('click','.char-search', function(e){
    ...
    $(this).addClass('active');
        $.ajax({
            url: '/events/wizard/search-users-nominal',
            type: 'GET',
            data: { char: $(this).attr('data-key') , event_id: $event_id, selected_users: selected, deleted_users: deleted, internal_list_id: internal_list_id, typeSearch: '$type', showAllUsers : '$showAllUsers'},
            success: function(data) {
                $('#container-result-search').html(data);
             }
        });
...
$('#show-all-users').click(function(e){
    ...
    let internal_list_id = $('#internal-list-id').val();
     ...
     $('#search-fields').hide();
    $.ajax({
        url: '/events/wizard/users-invited-nominal',
        type: 'GET',
        data: { id: $event_id, selected_users: users , deleted_users: users_deleted, internal_list_id: internal_list_id},
        success: function(data) {
...
<div class="col-md-6">
    <?php if ($communication) {
        $modelSearch->invitationName = $communication->title; ?>
        <div style="display:none"><?= $form->field($modelSearch, 'invitationName')->hiddenInput() ?></div>
    <?php } else { ?>
        <?= $form->field($modelSearch, 'invitationName')->textInput([
            'placeholder' => AmosEvents::t('amosevents', 'Assegna un titolo alla tua ricerca')
            // 'type' => 'search'
...
<div class="row mb-5">
    <div class="col-md-12">
        <?= Html::a(AmosEvents::t('amosevents', "Visualizza tutti gli utenti selezionati") . " (<span class='counter-selected'>$countInvitationUsers</span>) ", '#', [
            'id' => 'show-all-users',
...
<?php if (!$internalList->isNewRecord) {
    echo Html::hiddenInput('internal_list_id', $internalList->id, ['id' => 'internal-list-id']);
}
