...
<div class="box-widget-header">
    ...
    if ($thereAreEvents) {
        ...
        if ($moduleEvents->enableNewWizard) {
            $linkReadAll = '/events/wizard';
        }
        $textReadAll = AmosEvents::t('amosevents', '#addEvent');
        $checkPermNew = true;
        $canCreate = \Yii::$app->user->can('EVENT_CREATE');
    }
    ?>
    <?php if (!$checkPermNew || ($checkPermNew && $canCreate)) { ?>
            ...
            <div class="<?= ($thereAreEvents ? 'col-sm-8' : '') ?> col-xs-12 nop" id="event-calendar-pjax">
                ...
                <?php else: ?>
                    ...
                    <?php foreach ($eventsForCarousel as $event): ?>
                        ...
                        <div>
                            <a target="_blank"
                               href="<?= \open20\amos\events\utility\EventsUtility::getUrlLanding($event) ?>"
