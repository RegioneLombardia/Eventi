            ...
            if ($this->eventModel->has_tickets) {
                ...
                $itemsMittente['downloadedat'] = [
                    ...
                    'value' => function ($model) use ($eventInvitationModel) {
                        $result = "-";
                        $invitation = $eventInvitationModel::findOne(['user_id' => $model->user_id, 'event_id' => $this->eventModel->id]);
                        if ($invitation && !empty($invitation) && !empty($invitation->ticket_downloaded_at)) {
                            $dateTimeObject = new \DateTime($invitation->ticket_downloaded_at);
                            $result = $dateTimeObject->format('d/m/Y H:i');
                        }
                        return $result;
                    }
                ];
...
//            ];
            ...
            $itemsMittente['n_companions'] = [
                'value' => function ($model) use ($eventInvitationModel) {
                    $count = 0;
                    $invitation = $eventInvitationModel::findOne(['user_id' => $model->user_id, 'event_id' => $this->eventModel->id]);
                    if ($invitation) {
                        $count = $invitation->getCompanions()->count();
                    }
                    return $count;
                },
                'label' => AmosEvents::t('amosevents', "N. accompagnatori"),
        ...
        $widget = M2MWidget::widget([
            ...
            'actionColumnsButtons' => [
                ...
                'participantDetails' => function ($url, $model) use ($eventInvitationModel) {
                    $btn = '';
                    $invitation = $eventInvitationModel::find()->andWhere(['event_id' => $this->eventModel->id])->andWhere(['user_id' => $model->user_id])->one();
                    if (!empty($invitation)) {
                        ...
                        if (EventsUtility::hasPrivilegesLoggedUser($this->eventModel)) { // || Yii::$app->user->id == $model->user_id) {
                            $btn = Html::a(
                                ...
                                $url,
                                ['class' => 'btn btn-tertiary', 'title' => AmosEvents::txt('Visualizza scheda partecipante'), 'data-pjax' => '0',]);
                        }
                    }
                    return $btn;
                },
...
JS
        ...
        if ($this->enableAdditionalButtons && $this->checkManager()) {
            $this->view->registerJs(<<<JS
                $('#m2m-widget-btn-additional-associate').hide();
