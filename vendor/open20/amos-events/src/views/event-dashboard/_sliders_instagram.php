...
if (!$landing->isNewRecord) {
    $max = SiteManagementSliderElem::find()->andWhere(['slider_id' => $landing->instagram_video_slider_id])->max("site_management_slider_elem.order");
    $min = SiteManagementSliderElem::find()->andWhere(['slider_id' => $landing->instagram_video_slider_id])->min("site_management_slider_elem.order");
    $eventModel = $model;
    ?>
    <?=
    Html::a(
        ...
        [
            ...
            'id' => $landing->instagram_video_slider_id,
            'urlRedirect' => urlencode('/events/event-dashboard/landing-manage-contents?id=' . $model->id),
//            'useCrop' => true,
            'slider_type' => \amos\sitemanagement\models\SiteManagementSliderElem::TYPE_INSTAGRAM_VIDEO,
//            'cropRatio' => 1.7
        ],
...
//                'data-confirm' => AmosEvents::t('amosevents', 'Stai per lasciare la pagina, assicurarsi di aver salvato dati. Proseguire?')
    ...
    echo \open20\amos\core\views\AmosGridView::widget([
        'dataProvider' => $dataProviderSliderElemInstagram,
        'columns' => [
            [
                'label' => 'Ordina',
