...
$('#switch-logo-email').on('switchChange.bootstrapSwitch', function(){
        if($(this).is(':checked')){
            $('#container-logo-email').show();
        }else {
            $('#container-logo-email').hide(); 
        }
});
JS;
$this->registerJs($js);
...
<div id="event-images" style="<?= $hideImage ?>">
    <div class="immagine-landing my-4">
        ...
        <?php
        if (!empty($model->eventLogoEmail)) {
            $hideLogoEmail = '';
            $value = 1;
        } else {
            $hideLogoEmail = 'display:none';
            $value = 0;

        } ?>
        <div class="row variable-gutters my-4">
            <div class="col-md-12 pb-3">
                <label><?= AmosEvents::t('amosevents', "Vuoi personalizzare l'immagine per le notifiche email?") ?></label>
                <?php echo \kartik\widgets\SwitchInput::widget([
                    'name' => 'switch-logo-email',
                    'value' => $value,
                    'options' => ['id' => 'switch-logo-email' ],
                    'pluginOptions' => [
                        //'size' => 'mini',
                        //                    'onText' => AmosIcons::show('check-circle'),
                        //                    'offText' => AmosIcons::show('circle-o')
            ...
            <div id="container-logo-email" class="col-md-12" style="<?= $hideLogoEmail ?>">
                <div>
                    <?=
                    $form->field($model, 'eventLogoEmail')->widget(
                        \open20\amos\attachments\components\CropInput::classname(),
                        [
                            'jcropOptions' => ['aspectRatio' => '1.7', 'placeholder' => AmosEvents::t('amosevents', 'Per una visualizzazione ottimale, carica un\'immagine rettangolare 1920x1080 pixel.')],
                        ]
                    )->label(AmosEvents::t('amosevents', "Immagine dell'evento per Notifiche Email"))
                    ?>
                </div>
            </div>
        </div>
