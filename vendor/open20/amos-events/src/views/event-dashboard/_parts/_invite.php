...
$js = <<<JS
    ...
    // function search
     ...
     $('input[name="UserEventSearch[type]"]').click(function(){
          ...
          console.log(value);
          if(value == 3){
              $('#search-nominal-selection').show();
               $('#search-tags').hide();
               $('.dynamic-search-id').hide();
          }else if(value == 2){
              $('#search-tags').show();
               $('.dynamic-search-id').show();
              $('#search-nominal-selection').hide();
          } else {
              $('#search-tags').hide();
              $('.dynamic-search-id').show();
              $('#search-nominal-selection').hide();
        ...
        <div class="py-4 d-flex title-substeps affix-top">
            ...
            <h5 class="font-weight-bold ">
                ...
                <a href="#" data-toggle="tooltip" data-html="true"
                   title="<?= AmosEvents::t('amosevents', "Ricerca gli utenti da invitare all'evento e assegna un titolo alla tua ricerca. Puoi invitare solo gli utenti già presenti nel tuo database. <br><br><i>nota:</i> Il database delle Direzioni Generali raccoglie gli utenti importati e gli utenti registrati ad un evento precedente.") ?>">
        ...
        <div class="row variable-gutters">
            <div class="col-md-12">
                <?= $form->field($modelSearch, 'type')->radioList([
                    ...
                    UserEventSearch::SEARCH_TYPE_SELECT_PARTICIPANT => AmosEvents::t('amosevents', 'Selezione nominale'),
                ...
                if ($modelSearch->type == UserEventSearch::SEARCH_TYPE_SELECT_PARTICIPANT) {
                    $displayNoneNominal = '';
                    $displayNoneDynamicSearch = 'display:none';

                } ?>

                <div id="search-nominal-selection" style="<?= $displayNoneNominal ?>">
                    <?php
                    $controller = \Yii::$app->controller->id;
                    ?>
                    <?php if (!$internalList->isNewRecord) {
                        ?>
                        <?= Html::a(AmosEvents::t('amosevents', 'Modifica selezione utenti'), ["/events/$controller/nominal-invitation", 'id' => $model->id, 'list_id' => $internalList->id], [
                            'class' => 'btn btn-primary m-t-20',
                            'title' => AmosEvents::t('amosevents', 'Seleziona utenti')
                        ]) ?>
                    <?php } else { ?>
                        <?= Html::a(AmosEvents::t('amosevents', 'Seleziona utenti'), ["/events/$controller/nominal-invitation", 'id' => $model->id], [
                            'class' => 'btn btn-primary m-t-20',
                            'title' => AmosEvents::t('amosevents', 'Seleziona utenti')
                        ]) ?>
                    <?php } ?>
                ...
                <div class="dynamic-search-id" style="<?= $displayNoneDynamicSearch ?>">
                    <?php
                    $count = 1;
    ...
    if ($wizard == true) { ?>
        <div class="buttons row">
            <div class="bk-btnFormContainer col-xs-12 nop">
                ...
                <div class="col-sm-6 nop">
                    <button type="submit"
                            class="btn btn-primary pull-right wizard-widget-continue-btn"><?= AmosEvents::t('events', "Prosegui") ?></button>
                    <?= Html::a(AmosEvents::t('landing', "Vai al riepilogo"), ['/events/wizard/finish', 'id' => $model->id], [
