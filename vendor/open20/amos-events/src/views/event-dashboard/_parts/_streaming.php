...
<div class="row">
    <div class="col-md-6">
        <?php if (\Yii::$app->request->get('language') == 'en-GB') { ?>
            <strong><?= \Yii::t('amosevents', "Tipologia streaming")?></strong><br>
            <div class="mt-3">
                <?= $landing->streaming_type ? $landing->getLabelTypeStreaming()[$landing->streaming_type] : '' ?>
            </div>
            <?php echo $form->field($landing, 'streaming_type')->hiddenInput()->label(false); ?>
        <?php } else { ?>
