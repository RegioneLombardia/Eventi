...
<?php if ($model->eventType->limited_seats) { ?>
    <?php if ($model->isSeatManagement()) { ?>
        <?= AmosEvents::t('amosevents', '{x} Utenti registrati', [
            'x' => $totInvitedUsers - $totInvitedCompanions,
        ]) ?>
        <br>
        <?= AmosEvents::t('amosevents', '{x} Utenti registrati da gruppi', [
            'x' => $totInvitedCompanions,
        ]) ?>
        <br>
        <strong><?= AmosEvents::t('amosevents', 'Totale utenti registati {x}', [
                'x' => $totInvitedUsers,
            ]) ?>
        </strong>
        <br>
    ...
    <?php } else { ?>
        <?= AmosEvents::t('amosevents', '{x} Utenti partecipanti', [
            'x' => $totParticipants,
        ]) ?>
        <br>

        <?php if ($model->enable_companions) { ?>
            <?= AmosEvents::t('amosevents', '{x} Accompagnatori partecipanti', [
                'x' => $totConfirmedCompanions,
            ]) ?>
            <br>
        <?php } ?>
        ...
        <strong><?= AmosEvents::t('amosevents', 'Totale utenti registrati {x}', [
                'x' => $totParticipants + $totWaitingUsers + $totConfirmedCompanions,
            ]) ?></strong>
        <br>
        <br>
        <?= AmosEvents::t('amosevents', '<strong>TOTALE: {x} partecipanti su {y} posti disponibili</strong>', [
            'x' => ($totParticipants + $totConfirmedCompanions),
        ...
        if ($model->community_id) {
            ...
            ?>
            <?php $seatsRemaining = $seatsAvailable - ($totParticipants + $totConfirmedCompanions);
            ...
            <?php $percentage = round((($totParticipants + $totConfirmedCompanions) / $model->seats_available) * 100); ?>
