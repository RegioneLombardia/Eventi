    ...
    <div class="row variable-gutters">
        <div class="col-md-7 mr-auto order-2 order-md-1">
            ...
            <div class="d-flex align-items-center mb-5">
                ...
                <div>
                    <?= AmosEvents::t('amosevents', "Aspetto della landing page") . ': ' . "<strong>" . $titleTemplate . "</strong>" ?>
                    <br>
                </div>
            </div>
            ...
            <div class="d-flex align-items-center mb-5">
                ...
                <div>
                    <?= AmosEvents::t('amosevents', "La landing page sarà pubblicata il") . ': ' . "<strong>" . \Yii::$app->formatter->asDate($model->publication_date_begin) . "</strong>" ?>
                    <br>
                    <?= AmosEvents::t('amosevents', "all'indirizzo") . ': ' . "<strong>" . \open20\amos\events\utility\EventsUtility::getUrlLanding($model) . "</strong>" ?>
                    <br>
                </div>
            </div>
            ...
            <div class="d-flex align-items-center mb-5">
                ...
                <div>
                    <?= AmosEvents::t('amosevents', "Hai scelto il layout predefinito per le <strong>email</strong> da inviare agli utenti. Potrai modificarlo in qualunque momento.") . "<strong>" . "</strong>" ?>
                    <br>
                </div>
            </div>
            ...
            <div class="d-flex align-items-center mb-5">
                ...
                <div>
                    <?php if ($model->eventType->webmeeting_webex) {
                        $n_invited = $model->getEventSpecificUsers()->count();
                    } else {
                        $list = $model->getEventInternalLists()->one();
                        $n_invited = $list ? $list->n_user_found : 0;
                    } ?>
                    <?= AmosEvents::t('amosevents', "Hai salvato una lista di <strong>{n} utenti che puoi invitare.</strong>", ['n' => $n_invited]) ?>
                    <br>
                    <?= AmosEvents::t('amosevents', "Puoi invitare altri utenti in qualunque momento dalla dashboard.") ?>
                    <br>
                </div>
            </div>
            <div class="mt-5 pt-3">
...
DASHBOARD EVENTO altrimenti chiudi semplicemente questa finestra.</p>") ?>
    ...
    <div class="buttons mt-5">
        ...
        if ($model->eventType->webmeeting_webex) {
            echo \yii\helpers\Html::a(AmosEvents::t('amosevents', 'Indietro'), ['/events/wizard/participant-web-meeting', 'id' => $model->id], ['class' => 'btn btn-secondary']);
        } ?>
