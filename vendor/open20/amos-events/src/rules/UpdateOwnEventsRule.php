...
//            }
            if (!empty($model->getWorkflowStatus())) {
                if (((($model->created_by == $user) && $model->getWorkflowStatus()->getId() == Event::EVENTS_WORKFLOW_STATUS_DRAFT) || \Yii::$app->user->can('EventValidate', ['model' => $model])) && ($model->created_by == $user)) {
                    return true;
                }
            }
        }
