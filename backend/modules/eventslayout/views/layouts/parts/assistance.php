...
?>
<?php if (\Yii::$app->user->can('EVENT_DG_OPERATOR') || \Yii::$app->user->can('SUPER_USER_EVENT')|| \Yii::$app->user->can('ADMIN')) { ?>
    <?php if ((isset($assistance['enabled']) && $assistance['enabled']) || (!isset($assistance['enabled']) && isset(\Yii::$app->params['email-assistenza']))): ?>
        <div class="assistance">
            <a id="toggle-assistance" class="animated"
               href="<?= $isMail ? 'mailto:' . $mailAddress : (isset($assistance['url']) ? $assistance['url'] : '') ?>">
                <?php if ($newDashboard): ?>
                <div class="wrapper">
                    <?php endif; ?>
