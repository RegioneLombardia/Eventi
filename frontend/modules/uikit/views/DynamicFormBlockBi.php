...
<div>
    ...
    <?php if ($data['register_with_social']) { ?>
        <div id="spid-container" class="col-xs-12 m-t-30 mb-5 nop spid-container social-container text-center">
            <span style="width:100%;display:block;"><?= \Yii::t('app', "<strong>Accedi</strong> con idPC - Sisitema di Identity Provider di Regione Lombardia") ?></span>
            <a data-key="spid" class="btn btn-spid" title="Accedi con SPID"
               href="<?= \Yii::$app->params['platform']['frontendUrl'] . "/socialauth/shibboleth/endpoint" ?>">
                <span class="icon-logo-spid text-white"><?= \open20\amos\admin\AmosAdmin::t('amosadmin', "accedi con la tua identità digitale") ?></span>
            </a>
        </div>
        <?php if (!$dlSemplification) { ?>
            <div id="social-container" class="col-xs-12 m-t-30 mb-5 nop social-container text-center">
                <span style="width:100%;display:block;"><?= \Yii::t('app', "<strong>Registrati</strong> con i tuoi account social") ?></span>
    ...
    ?>
    <div class="col-xs-12 nop" style="<?= $hideFormClass ?>">
        <?php
        ...
        foreach ($attributes as $attribute) {
            ...
            <div<?= Uikit::attrs($item_attrs) ?>
                    class="<?= $attribute ?>_item <?= $inRiepilogo ? 'in-riepilogo' : '' ?>"
                    data-key="<?= $attribute . '-id' ?>" data-type="<?= $model->attributeTypes()[$attribute] ?>">
                <?php
                $filedItem = '';
                switch ($model->attributeTypes()[$attribute]) {
                    case 'hidden':
                        $filedItem = $form->field($model, $attribute, ['options' => ['style' => 'display:none']])->hiddenInput($model->getAttributeOptions($attribute))->label(false);
                        break;
                    case 'label':
                        $filedItem = "<div class='pull-right'>" . Html::tag(
                                'span',
                    ...
                    case 'attachmentsInput':
                        $filedItem = "<div class='upload-bi-design'>" . $form->field(
                                $model,
                                $attribute,
                                [
                            ...
                            )->label($model->getAttributeLabel($attribute)) . "</div>";
                        break;
                    case 'checkbox':
                    ...
                    case 'textarea':
                        ...
                        break;
                    case 'textEditor':
                        $filedItem = $form->field($model, $attribute, $model->getAttributeOptions($attribute))->widget(\open20\amos\core\forms\TextEditorWidget::className())
                            ->label($model->getAttributeLabel($attribute),['class' => 'position-relative'])->hint(false);
                        break;
                    case 'date':
                        $filedItem = $form->field($model, $attribute, $model->getAttributeOptions($attribute))->widget(
                    ...
                    case 'url':
                        ...
                        break;
                    default:
                        $filedItem = $form->field($model, $attribute, $model->getAttributeOptions($attribute))->label($model->getAttributeLabel($attribute));
                        break;
                }
                echo $filedItem;
                ?>
        ...
        <div class="uk-form-controls">
            <?=
            Html::submitButton(
                !empty($data['submitlabel']) ? $data['submitlabel'] : 'Submit',
                ['id' => 'submit-form-btn', 'class' => 'btn btn-primary']
            )
            ?>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal2Title"><?= $data['confirmModalTitle'] ?></h5>
