    ...
    protected function buildModel()
    {
        ...
        if (is_array($data['items'])) {
            foreach ($data['items'] as $item) {
                ...
                if ($item['required']) {
                    if ($item['field'] == 'privacy') {
                        $model->addRule($item['field'], 'required', ['requiredValue' => 1, 'message' => "É necessario accettare la privacy"]);
                    } else {
                        $model->addRule($item['field'], 'required');
                    }
                }

//                if ($item['field'] == 'fiscal_code') {
//                    $model->addRule($item['field'], \open20\amos\core\validators\CFValidator::className());
//                }
                ...
                if ($item['field'] == 'telefon') {
                    $model->addRule($item['field'], \open20\amos\core\validators\PhoneValidator::className());
                }
                if ($item['field'] == 'n_companions') {
                    $model->addRule($item['field'], 'safe');
                }

                // obbligatorietà selezionata dal backend - Configurazioni landing
                if(in_array($item['field'], $data['required_fields'])){
                    $model->addRule($item['field'], 'required');
                    $model->addRule($item['field'], 'safe');
                }
            }
