...
class SitemapController extends Controller {
    ...
    private function buildSitemapfile($sitemapFile) {
        ...
        if ($this->module->module->hasModule('cms')) {
            ...
            foreach (Yii::$app->menu->find()->andWhere(['parent_nav_id' => 0])->all() as $nav) {
                /** @var Nav $nav */
                $urls = [];
                //foreach($nav->navItems as $navItem) {
                /** @var NavItem $navItem */
                $urls[$navItem->lang->short_code] = ((strpos($nav->link, Yii::$app->request->hostInfo) !== false) ? '' : Yii::$app->request->hostInfo) . $nav->link;
                //}
        ...
        /*  if (Yii::$app->getModule('backendobjects')) {
          ...
          if ($searchClass != 'common\modules\transformermanagers\ElasticModelSearch') {
