...
class CmsapiController extends Controller
{
    ...
    public function actionNews()
    {
        ...
        foreach ($dataprovider->query->all() as $model) {
            $url = \Yii::$app->getModule('backendobjects')::getSeoUrl($model->getPrettyUrl(), 572, false, false);
    ...
    public function actionCommunity()
    {
        ...
        $dataprovider      = $discussioniSearch->cmsSearch(Yii::$app->request->get(), $limit);
        ...
        foreach ($dataprovider->query->all() as $model) {
            $url      = \Yii::$app->getModule('backendobjects')::getSeoUrl($model->getPrettyUrl(), 597, false, false);
            $itemMenu .= '<li>
            <a class="" title="'.$model->titolo.'" href="'.$url.'">';
            $itemMenu .= $model->titolo;
            $itemMenu .= '</a> 
    ...
    public function actionRedazione()
    {
        ...
        foreach ($dataprovider->query->all() as $model) {
            $url      = \Yii::$app->getModule('backendobjects')::getSeoUrl($model->getPrettyUrl(), 573, false, false);
            $itemMenu .= '<li>
            <a class="" title="'.$model->titolo.'" href="'.$url.'">';
            if (isset($model->data_pubblicazione) && !empty($model->data_pubblicazione)) {
                $itemMenu .= $model->titolo.' | '.Yii::$app->getFormatter()->asDate($model->data_pubblicazione);
    ...
    public function actionCreatePage()
    {
        ...
        try {
            ...
            $myfile = fopen("error_luya_front.txt", "a") or die("Unable to open file!");
                    ...
                    if ($postCmsPage->load($request->post(), '') && $postCmsPage->validate()) {
                        ...
                        if (!empty($postCmsPage->nav_id)) {
                            ...
                            if (!is_null($model)) {
                                $result->nav_id      = $model->id;
                                $result->preview_url = $model->getPreviewUrl();
                                $landing             = new CmsLandigBuilder([], $postCmsPage);
