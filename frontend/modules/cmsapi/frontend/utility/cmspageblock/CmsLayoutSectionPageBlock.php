...
//        }
     ...
     * @return bool|\luya\admin\image\Item|\luya\Exception|null
     * @throws \luya\Exception
     */
    protected function createCmsImage(PostCmsCreatePage $postCmsPage)
    {
        if (!array_key_exists($postCmsPage->event_data->url_image, self::$imageUrl)) {
            ...
            $file = $storage->addFile($outPutFilePath, $fileName);

            $image = $storage->addImage($file->id);
            if (!is_null($image)) {
                self::$imageUrl[$postCmsPage->event_data->url_image] = $image;
                unlink($outPutFilePath);
            }
        } else {
            $image = self::$imageUrl[$postCmsPage->event_data->url_image];
        }
